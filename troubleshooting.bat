...
python -c "import ai_meta_generator" >nul 2>&1 || (echo ai_meta_generator module not found && exit /b 1)
...
python -m ai_meta_generator.cli --config config.ini --template-file template.json || (echo Header generation failed && exit /b 1)
